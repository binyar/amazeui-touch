...
const DATA = {
  ...
  "香港": {"香港特别行政区": ["香港岛", "新界", "九龙", "离岛"]},
  "澳门": {"澳门特别行政区": ["澳门半岛", "氹仔", "路环"]},
  "台湾": {
...
class Address extends Component {
  ...
  createItems(type) {
    ...
    switch (type) {
      ...
      case 'city':
        ...
        break
      case 'district':
        for (let value of DATA[province][city]) {
          let item = {}
          item.text = value
          item.value = value
          items.push(item)
  ...
  createCols() {
    ...
    let cols = []
    let provinces = {
      items: this.createItems('province'),
      value: province,
      width: 5,
    }
    let cities = {
      items: this.createItems('city'),
      value: city,
      width: 6,
    }
    let districts = {
      items: this.createItems('district'),
      value: district,
      width: 5
    }
    let divider = {
